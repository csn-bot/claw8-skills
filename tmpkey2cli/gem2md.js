...
// @version      3.1.1-d1bridge
// @description  Export your ChatGPT, Claude, Copilot, Gemini or Grok chat into Markdown or JSON. Markdown export can POST to tmpkey2cli dayone-cli-bridge (localhost → dayone CLI).
...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/541051/ChatGPT%20%20Claude%20%20Copilot%20%20Gemini%20%20Grok%20AI%20Chat%20Exporter%20by%20RevivalStack.user.js
// @updateURL https://update.greasyfork.org/scripts/541051/ChatGPT%20%20Claude%20%20Copilot%20%20Gemini%20%20Grok%20AI%20Chat%20Exporter%20by%20RevivalStack.meta.js
// ==/UserScript==
...
(function () {
  ...
  // --- Global Constants ---
  const EXPORTER_VERSION = "3.1.1-d1bridge";
  ...
  const GM_DAYONE_BRIDGE_URL = "aiChatExporter_dayoneBridgeUrl";
  const GM_DAYONE_BRIDGE_TOKEN = "aiChatExporter_dayoneBridgeToken";
  const GM_DAYONE_BRIDGE_ENABLED = "aiChatExporter_dayoneBridgeEnabled";
  const DAYONE_BRIDGE_URL_DEFAULT = "http://127.0.0.1:8765/dayone";
  ...
  GM_registerMenuCommand("Day One bridge: set shared token", () => {
    const cur = GM_getValue(GM_DAYONE_BRIDGE_TOKEN, "");
    const v = prompt(
      "Paste the same secret as DAYONE_BRIDGE_TOKEN in tmpkey2cli/.env.\n" +
        "Stored only in Tampermonkey on this browser.",
      cur || ""
    );
    if (v !== null) {
      GM_setValue(GM_DAYONE_BRIDGE_TOKEN, String(v).trim());
      alert("Day One bridge token saved.");
    }
  });

  GM_registerMenuCommand("Day One bridge: set POST URL", () => {
    const cur = GM_getValue(GM_DAYONE_BRIDGE_URL, DAYONE_BRIDGE_URL_DEFAULT);
    const v = prompt(
      "Local bridge POST URL (default http://127.0.0.1:8765/dayone):",
      cur
    );
    if (v !== null && String(v).trim()) {
      GM_setValue(GM_DAYONE_BRIDGE_URL, String(v).trim());
      alert("Bridge URL saved.");
    }
  });

  GM_registerMenuCommand("Day One bridge: toggle send on Export MD", () => {
    const cur = GM_getValue(GM_DAYONE_BRIDGE_ENABLED, true);
    const next = !cur;
    GM_setValue(GM_DAYONE_BRIDGE_ENABLED, next);
    alert(
      next
        ? "Day One bridge: ON — Export MD downloads the file and POSTs to the bridge."
        : "Day One bridge: OFF — Export MD only downloads the file."
    );
  });
  ...
  /**
   * POST Markdown to tmpkey2cli dayone-cli-bridge (requires GM_xmlhttpRequest for HTTPS→HTTP localhost).
   */
  const DayOneBridge = {
    postMarkdown(subject, bodyMarkdown, tagsArray) {
      const enabled = GM_getValue(GM_DAYONE_BRIDGE_ENABLED, true);
      if (!enabled) return;

      const tokenStored = GM_getValue(GM_DAYONE_BRIDGE_TOKEN, "");
      if (typeof tokenStored !== "string" || !tokenStored.trim()) {
        console.warn(
          "AI Chat Exporter: Day One bridge token not set (Tampermonkey menu → Day One bridge: set shared token)."
        );
        return;
      }

      const url = GM_getValue(GM_DAYONE_BRIDGE_URL, DAYONE_BRIDGE_URL_DEFAULT);
      const tags = Array.isArray(tagsArray)
        ? tagsArray.filter((t) => typeof t === "string" && t.trim())
        : [];
      const payload = JSON.stringify({
        subject: String(subject || "").trim() || "AI chat export",
      ...
      GM_xmlhttpRequest({
        ...
        headers: {
          ...
          "X-Bridge-Token": tokenStored.trim(),
        },
        data: payload,
        onload(resp) {
          let parsed = null;
          ...
          if (resp.status >= 200 && resp.status < 300 && parsed && parsed.ok) {
            console.log("Day One bridge: ok", parsed);
            return;
          }
          const err =
            (parsed && parsed.error) ||
            (resp.responseText && resp.responseText.slice(0, 500)) ||
            `HTTP ${resp.status}`;
          console.error("Day One bridge failed:", err);
          alert(
            `Day One bridge failed (${resp.status}): ${err}\n\nIs ./start-bridge.sh running? Does the token match .env?`
          );
        },
        onerror() {
          console.error("Day One bridge: network error");
          alert(
            "Day One bridge: could not reach localhost. Start tmpkey2cli/start-bridge.sh and check the POST URL."
          );
        },
      });
    },
  ...
  const ChatExporter = {
    ...
    formatToMarkdown(chatData, turndownServiceInstance) {
      ...
      const fencePipeTables = (md) => {
        ...
        const countPipes = (s) => (s.match(/\|/g) || []).length;
        const isFenceLine = (s) => /^\s*```/.test(s);
        const isPipeLike = (s) => countPipes(s) >= 2;
        const isSeparatorLike = (s) =>
          /^\s*\|?\s*:?-{2,}:?\s*(\|\s*:?-{2,}:?\s*)+\|?\s*$/.test(s);

        const flushTable = () => {
          if (!buf.length) return;
          // Day One-friendly: fenced copy, then a raw copy between separators.
          out.push("```");
          out.push(...buf);
          out.push("```");
          out.push("___");
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          const looksLikeTableLine =
            trimmed.length > 0 && (isPipeLike(line) || isSeparatorLike(line));
      ...
      const collapsePromptIfHardWrapped = (text) => {
        const s = String(text || "");
        const newlineCount = (s.match(/\n/g) || []).length;
        const blankLineCount = (s.match(/\n\s*\n/g) || []).length;
        const looksHardWrapped =
          newlineCount >= 6 || (newlineCount >= 3 && blankLineCount >= 2);
        if (!looksHardWrapped) return s.trim();
        return s
          .replace(/\s*\n+\s*/g, " ")
          .replace(/\s+/g, " ")
          .trim();
      ...
      chatData.messages.forEach((msg) => {
        if (msg.author === "user") {
          ...
          const prompt = collapsePromptIfHardWrapped(msg.contentText);
          content +=
          ...
          markdownContent = fencePipeTables(markdownContent);
      ...
      // Day One-friendly: no TOC, no "top" footer, and put metadata at the bottom.
      const finalOutput =
        `# ${chatData.title}\n\n` + content.trim() + "\n\n" + yaml;
        ...
        if (format === "markdown") {
          const subject =
            (chatDataForExport.title &&
              String(chatDataForExport.title).trim()) ||
            String(chatDataForExport._raw_title || "AI chat export").trim();
          DayOneBridge.postMarkdown(
            subject,
            fileOutput,
